...
pub async fn process_sign(args: SignArgs) -> Result<()> {
    ...
    if let Some(mint_id) = args.mint {
        ...
        let account_keys = match solana_cluster {
            ...
            Cluster::Mainnet => {
                let client = RpcClient::new("https://patient-proud-dew.solana-mainnet.quiknode.pro/b6429aaaf067e04ae3d433fd41a608726a1ee990/");
                let (creator, _) = find_candy_machine_creator_pda(&candy_machine_id);
                let creator = bs58::encode(creator).into_string();
                get_cm_creator_accounts(&client, &creator, 0)?
                // NOTE: There is a dependency conflict with the solana_transaction_crawler
                // so I'm just disabling this code for now and using the above code to
