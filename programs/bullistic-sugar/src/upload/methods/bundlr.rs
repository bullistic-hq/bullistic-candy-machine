...
impl BundlrMethod {
    ...
    async fn fund_bundlr_address(
        ...
        println!("Funding Bundlr address:");
        println!("  -> payer pubkey: {}", payer_pubkey);
        println!("  -> bundlr recipient pubkey: {}", bundlr_address);
        println!(
            "  -> lamports paid: {} (◎ {})",
            amount,
            amount as f64 / LAMPORTS_PER_SOL as f64
        );
        println!("  -> bundlr node: {}", node);
        ...
        println!("{} {sig}", style("Payment Transaction Signature:").bold());
    ...
    async fn prepare(
        ...
        if lamports_fee > balance {
            BundlrMethod::fund_bundlr_address(
                ...
                // NOTE: For some reason only passing in lamports_fee - balance here
                // is insufficient to pay bundlr and tends to fail with some '402
                // payment required' error. Increasing the fee a bit seems to work.
                (lamports_fee - balance) * 2,
            )
            .await?;
