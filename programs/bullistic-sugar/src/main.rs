...
use clap::Parser;
use console::style;
use bullistic_sugar_cli::{
    bundlr::{process_bundlr, BundlrArgs},
    clear_merkle_allowlist::{clear_merkle_allowlist, ClearMerkleAllowlistArgs},
    ...
    merkle_allowlist::{process_merkle_allowlist, ProcessMerkleAllowlistArgs},
...
fn setup_logging(level: Option<EnvFilter>) -> Result<()> {
    ...
    let current_directory_env = env::var("CURRENT_DIR")
        .expect("Expected to find CURRENT_DIR environment variable defined.");
    let mut log_path = PathBuf::from(current_directory_env);
    log_path.push(".sugar-cli-run/sugar.log");

    let file = OpenOptions::new()
        .write(true)
        .create(true)
        .open(&log_path)
        .unwrap_or_else(|_| panic!("Could not create sugar log file at path: {:?}", log_path));
...
async fn run() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::SetMerkleAllowlist {
            ...
            process_merkle_allowlist(ProcessMerkleAllowlistArgs {
                cache,
                candy_machine,
                config,
                keypair,
        ...
        Commands::ClearMerkleAllowlist {
            ...
            clear_merkle_allowlist(ClearMerkleAllowlistArgs {
                cache,
                candy_machine,
                config,
                keypair,
