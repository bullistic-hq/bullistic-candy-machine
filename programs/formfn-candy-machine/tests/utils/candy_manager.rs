...
use formfn_candy_machine::constants::{FREEZE_FEATURE_INDEX, FREEZE_LOCK_FEATURE_INDEX};
use formfn_candy_machine::{
    constants::BOT_FEE,
...
pub struct CandyManagerBuilder {
    ...
    formfn_authority: Option<Keypair>,
...
impl CandyManagerBuilder {
    ...
    pub fn set_formfn_authority(mut self, formfn_authority: Keypair) -> CandyManagerBuilder {
        self.formfn_authority = Some(formfn_authority);
        self
    }
...
impl Clone for CandyManager {
    fn clone(&self) -> Self {
        CandyManager {
            ...
            formfn_authority: clone_keypair(&self.formfn_authority),
...
impl CollectionInfo {
    ...
    pub async fn init(
        ...
        let metadata_info = MetadataManager::new(&formfn_authority);
        metadata_info
            .create_v2(
                context,
...
impl FreezeInfo {
    pub fn new(set: bool, candy_machine: &Pubkey, freeze_time: i64, mint: Pubkey) -> Self {
        ...
        let pda = Pubkey::find_program_address(seeds, &formfn_candy_machine::ID).0;
...
impl CandyManager {
    ...
    pub async fn init(
        ...
        let formfn_authority = formfn_authority.unwrap_or(Keypair::new());
        let default_creator_authority = if collection {
            clone_keypair(&formfn_authority)
        } else {
            Keypair::new()
        };
    ...
    pub async fn mint_and_assert_successful(
        ...
        if let Some(change) = balance_change {
            let is_authority_minting =
                cmp_pubkeys(&self.formfn_authority.pubkey(), &self.minter.pubkey());
        ...
        new_nft.authority = clone_keypair(&self.formfn_authority);
        Ok(new_nft)
    }
