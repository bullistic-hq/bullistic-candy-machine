...
use bullistic_candy_machine::constants::{
...
async fn freeze_flow_with_spl_token() {
    ...
    candy_manager
        .thaw_nft(
            context,
            &new_nft,
            &clone_keypair(&candy_manager.bullistic_authority),
        )
        .await
        .unwrap_err();
    candy_manager.assert_frozen(context, &new_nft).await;
...
async fn unlock_funds() {
    ...
    candy_manager.unlock_funds(context).await.unwrap();
    let post_balance = get_balance(context, &candy_manager.bullistic_authority.pubkey()).await;
