...
# RPC="https://patient-proud-dew.solana-mainnet.quiknode.pro/b6429aaaf067e04ae3d433fd41a608726a1ee990"
# STORAGE="bundlr"
#
# ENV_URL="devnet"
...
USE_MERKLE_ALLOWLIST_FLAG=$1

ALLOWLIST_TYPE=""
if [[ "$USE_MERKLE_ALLOWLIST_FLAG" == "use-merkle-allowlist" ]]; then
  ALLOWLIST_TYPE="merkle"
else [[ "$ALLOWLIST_TYPE" == "use-spl-allowlist" ]];
  ALLOWLIST_TYPE="spl"
fi
...
# settings for running with a local validator
function local_settings() {
    ...
    AWS_BUCKET="null"
    ENV_URL="localhost"
    NFT_STORAGE_TOKEN="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweDVkYjM0M2JjRjVhMDgxODBDM2IwZTQyRmUxOTdjODIyMjMwY2JDNTkiLCJpc3MiOiJuZnQtc3RvcmFnZSIsImlhdCI6MTY2MjkzMTY2MjIxOCwibmFtZSI6InN1Z2FyLWNsaS10ZXN0In0.j_WWsO2pZjvKVbnaseq9wXghTy2wfD1oI7xxZdzdp9s"
    CLEAR_MERKLE_ALLOWLIST="n"
}

# default test templates
function devnet_default_settings() {
    MANUAL_CACHE="n"
    ITEMS=25
    MULTIPLE=1
...
echo "To run the default test option with a local validator, run 'yarn test-local-spl' or 'yarn test-local-merkle'."
...
echo "2. devnet"
echo "3. mainnet-beta"
echo "4. hidden settings"
echo "5. manual cache"
echo "6. sugar launch"

if [ -f "$RESUME_FILE" ]; then
    echo "7. previous run ($(RED "resume"))"
    echo -n "$(CYN "Select test template [1-7]"): "
else
    echo -n "$(CYN "Select test template [1-6]"): "
fi
...
while getopts r:p flag; do
    case "${flag}" in
        r) RPC=${OPTARG} ;;
        p) SUGAR_BIN="cargo run --release --bin formfn_sugar --" ;;
        *) ;;
    esac
done
...
if [ -z ${CLEAR_MERKLE_ALLOWLIST+x} ]; then
    echo ""
    echo -n "$(CYN "Test clearing merkle allowlist after creation [Y/n]") (default 'Y'): "
    read CLEAR_MERKLE_ALLOWLIST
    if [ -z "$CLEAR_MERKLE_ALLOWLIST" ]; then
        CLEAR_MERKLE_ALLOWLIST="Y"
    fi
fi
...
CREATORS="[{\"address\": \"$WALLET_ADDRESS\", \"share\": 100}]"
...
SPL_ALLOWLIST_SETTINGS="null"
if [ "$ALLOWLIST_TYPE" = "spl" ]; then
    SPL_TOKEN_MINT=$(cat allowlist-config/cli-spl-allowlist-config.json | jq '.splTokenAllowlistMint')
    SPL_ALLOWLIST_SETTINGS="{\"mint\":$SPL_TOKEN_MINT,\"mode\":\"burnEveryTime\"}"
fi
...
cat >$CONFIG_FILE <<-EOM
{
    ...
    "splToken": null,
    "publicSaleStartTime": "$(gdate -d "+1 days" "+%Y-%m-%dT%T%z" | sed "s@^.\{22\}@&:@")",
    "allowlistSaleStartTime": "$(gdate -d "-1 days" "+%Y-%m-%dT%T%z" | sed "s@^.\{22\}@&:@")",
    "splTokenAllowlistSettings": $SPL_ALLOWLIST_SETTINGS,
    ...
    "sequentialMintOrderEnabled": false,
    "creatorAuthority": "$WALLET_ADDRESS",
    "creators": $CREATORS
}
EOM
...
if [ "$LAUNCH" = "Y" ]; then
    ...
    if [ "$ALLOWLIST_TYPE" = "merkle" ]; then
        echo ""
        CYN "4. Uploading merkle allowlist"
        echo ""
        MAG ">>>"
        setMerkleAllowlist
        MAG "<<<"
        ...
        if [ "${CLEAR_MERKLE_ALLOWLIST}" = "Y" ]; then
            echo ""
