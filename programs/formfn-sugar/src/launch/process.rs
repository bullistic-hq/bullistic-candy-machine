...
pub async fn process_launch(args: LaunchArgs) -> Result<()> {
    ...
    let deploy_args = DeployArgs {
        ...
        cache: args.cache.clone(),
        interrupted: args.interrupted.clone(),
        use_merkle_allowlist: args.use_merkle_allowlist,
        merkle_allowlist_config: args.merkle_allowlist_config.clone(),
    };

    process_deploy(deploy_args).await?;

    if args.use_merkle_allowlist {
        println!("\n{} sugar merkle-allowlist\n", style(">>>").magenta());

        let merkle_allowlist_args = ProcessMerkleAllowlistArgs {
            config: args.config.clone(),
            keypair: args.keypair.clone(),
            rpc_url: args.rpc_url.clone(),
            cache: args.cache.clone(),
        ...
        process_merkle_allowlist(merkle_allowlist_args).await?;
    }
